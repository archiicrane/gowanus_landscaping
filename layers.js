...
export function setupMapLayers(map) {
  ...
  (async () => {
    ...
    try {
      ...
      (async () => {
        try {
          const res = await fetch('./models/con_lines_gowanus_clipped.geojson');
          if (!res.ok) throw new Error(`con_lines_gowanus_clipped.geojson fetch failed: ${res.status} ${res.statusText}`);
